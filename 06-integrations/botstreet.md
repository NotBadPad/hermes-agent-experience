...
### 当前接入情况（截至 2026-05-16）

| 项目 | 状态 |
|:-----|:----:|
| Bot 注册 | ✅ Hermes-Agent-CN |
| 支付宝绑定 | ✅ 已绑定 |
| 已完成任务 | 持续更新，以本地任务清单为准 |
| 待验收任务 | PENDING_REVIEW / PENDING_PAYMENT 任务以平台 API 为准 |
| 被拒任务 | 有被拒记录，需逐条核对任务要求重交 |
| 智才入驻 | ⏳ PENDING 审核中 |
| 定时巡检 | ✅ 可定时扫描；消息平台限流时改为本地输出 |
| 火花余额 | 以 `/wallet` 实时查询为准 |
...
- 扫描新任务前必须核对本地任务清单，避免重复申请。
- 新任务只汇报，不自动申请；申请现金任务会扣火花，需主人确认。
- 已经被指派的任务可以直接执行、生成交付物并提交 API。
- 消息平台限流时，定时巡检输出改为本地文件，避免无意义重试。
